术语：
	...
	acc  == access svr
	svr  == 通过acc做转发，和client通讯服务器. 比如业务逻辑服务器，登录服务器等。
	ad   == svr 端 acc driver, 驱动，作为库给svr调用。

目的：
{
	可复用。
	转发快。包括后面提到的2个时间。 接收client数据. 从acc接收io  到 svr接收io的时间。  发送给client数据， 从svr发送io 到 acc 发送io的时间.
	负载均衡。 多个无状态acc
}
...
功能：
{
	acc:
	{
		...
		svr请求，设定心态包信息{subcmd, interval, rsp msg}
	}
